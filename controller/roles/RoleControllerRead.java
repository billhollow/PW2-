...
public class RoleControllerRead extends HttpServlet {
						...
						if(request.getParameter("action").equals("reading")){
							...
							for(int i = 0;i<roles.size() && roles.get(i) != null;i++){
								...
								if(roles.get(i).getId()== Long.parseLong((request.getParameter("roleId")))){
									...
									RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Views/Roles/roleRead.jsp");
