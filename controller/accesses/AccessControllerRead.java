...
public class AccessControllerRead extends HttpServlet {
						...
						if(request.getParameter("action").equals("reading")){
							...
							for(int i = 0;i<accesses.size() && accesses.get(i) != null;i++){
								if(accesses.get(i).getId()== Long.parseLong((request.getParameter("accessId")))){
									...
									RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Views/Accesses/accessRead.jsp");
