...
public class AccessControllerCreate extends HttpServlet {
				...
				if( rSearch.isEmpty()){
					...
					if(aSearch.isEmpty()){
						...
						if(request.getParameter("action").equals("creating")){
							...
							RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Views/Accesses/accessCreate.jsp");
							...
							a = new Access(roles.get(i).getId(),resources.get(j).getId());
