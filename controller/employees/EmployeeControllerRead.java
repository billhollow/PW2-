...
public class EmployeeControllerRead extends HttpServlet {
					...
					if(aSearch.isEmpty()){
						...
						if(request.getParameter("action").equals("reading")){
							...
							for(int i = 0;i<employees.size() && employees.get(i) != null;i++){
								System.out.println(employees.get(i).getId()+" no es igual a " + request.getParameter("employeeId"));
								if(employees.get(i).getId()== Long.parseLong((request.getParameter("employeeId")))){
									request.setAttribute("employee", employees.get(i));
									RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Views/Employees/employeeRead.jsp");
