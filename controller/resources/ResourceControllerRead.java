...
public class ResourceControllerRead extends HttpServlet {
						...
						if(request.getParameter("action").equals("reading")){
							...
							for(int i = 0;i<resources.size() && resources.get(i) != null;i++){
								if(resources.get(i).getId()== Long.parseLong((request.getParameter("resourceId")))){
									...
									RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Views/Resources/resourceRead.jsp");
