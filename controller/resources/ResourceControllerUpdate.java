					...
					if(aSearch.isEmpty()){
						...
						if(request.getParameter("action").equals("updating")){
							...
							for(int i = 0;i<resources.size() && resources.get(i) != null;i++){
								System.out.println(resources.get(i).getId()+" no es igual a " + request.getParameter("resourceId"));
								if(resources.get(i).getId()== Long.parseLong((request.getParameter("resourceId")))){
									request.setAttribute("resource", resources.get(i));
									RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Views/Resources/resourceUpdate.jsp");
